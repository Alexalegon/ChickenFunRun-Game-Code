 ...
 */
public class PositiveXRunningAlg implements RunningAlg{
    public static final PointObjectSettings.Direction direction = PointObjectSettings.Direction.XPOSITIVE;
...
float length;
    ...
    public PositiveXRunningAlg() {
    }


public PositiveXRunningAlg(PointSystem pointSystem,ObstacleSystem obstacleSystem,PrefabPool pool) {
    ...
    public int runTravelingRoad(Vector3f characterLoc, Vector3f checkPoint) {
        ...
        //System.out.println("CharacterLoc z is:  " + characterLoc.z);
        //System.out.println("CheckPoint  z is    " + checkPoint.z);
        //System.out.println("ChecjhPoint y is:   " + checkPoint.y);
    ...
    public Vector3f calculateRoadChangingFlag(float offset, float dimensions, Vector3f firstTile) {
        Vector3f vec = new Vector3f(length - offset - dimensions,firstTile.z - offset,offset);
        //for(int i = 0; i < 10; i++){
            //System.out.println("Z location of tile is:  " + firstTile.z);
            //System.out.println("Offset of tile is:  " + offset);
            //System.out.println("Vec result is:  " + vec.z);
        //}
    ...
    public float calculateDistanceTraveled(Vector3f characterLoc, Vector3f startingLoc, float offset) {
       if(characterLoc.x <= startingLoc.x)
           return 0;
       float endSpot = characterLoc.x - startingLoc.x;
       endSpot += offset;
       return endSpot;
    }
    ...
    public void fillRoadTiles(BulletAppState bulletAppState,CollisionShape shape,Node node, Road road, Vector3f firstTile, int numberOfTiles, float dimensions) {
        ...
        for(int i = 0;i < numberOfTiles;i++){
                  Node nodes = (Node) node.clone();//nodes.setLocalRotation(RotationQuats.ROTATIONQUAT90);
    ...
    public void showRoad(boolean initiate,Node node, BulletAppState bulletAppState, Road road, Vector3f location) {
        ...
        for(Node nodes: prefab.tileList){
            ...
            nodes.getControl(RigidBodyControl.class).setPhysicsRotation(RotationQuats.ROTATIONQUAT90);
...
float previousAddition = 0;
    ...
    public void setWall(Node node, Vector3f location, PointObjectSettings.Direction nextDirection) {
        ...
        if(nextDirection.equals(PointObjectSettings.Direction.ZPOSITIVE)){
    ...
    public String getTurnDirection(PointObjectSettings.Direction nextDirection) {
        if(nextDirection.equals(PointObjectSettings.Direction.ZNEGATIVE))
            return "left";
        return "right";
    }
